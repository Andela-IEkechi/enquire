...
RSpec.describe "doctor_profiles/new", :type => :view do
  before(:each) do
    assign(:doctor_profile, DoctorProfile.new(
      :doctor => nil,
      :gender => "MyString",
      :address => "MyText",
      :phone_number => 1,
  ...
  it "renders new doctor_profile form" do
    render

    assert_select "form[action=?][method=?]", doctor_profiles_path, "post" do
